...
class ActionDraftInfo(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        ...
        logger.info(f"action_draft_info called: player={player_name}, season={season}")

        # If no player slot, try to extract from message text
        if not player_name:
            player_name = self._extract_player_from_text(tracker.latest_message.get("text", ""))
            logger.info(f"Extracted player from text: {player_name}")

        # If no season slot, try to extract from message text
        if not season:
