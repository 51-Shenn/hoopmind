...
class NBAQueryEngine:
    ...
    def _season_candidates(season: Any) -> list[str]:
        ...
        This is useful because some datasets store seasons as
        starting years such as 2024.
        """
        if season is None:
            return []
    ...
    def query(
        ...
        try:
            ...
            # ------------------------------------------------
            PLAYER_NEEDED = {"player_information", "player_career_totals"}
            TEAM_NEEDED = {
                "team_information",
                "team_summary",
                "team_opponent_stats",
            ...
            if intent == "compare_players":
                have = [
                    p
                    for p in (params.get("player1"), params.get("player2"))
                    if p
                ]
    ...
    def _player_stat_query(
        ...
        # not compare latest-season averages.
        # --------------------------------------------------------
        use_totals = bool(
            compare
            and query_text
            and re.search(
                r"\bcareer\b|\ball[- ]time\b|\btotals?\b",
                query_text.casefold(),
            )
        )
        if use_totals:
        ...
        return QueryResult(
            True,
            {
                ...
                "measure": "career totals" if use_totals else "per game",
            },
        )
